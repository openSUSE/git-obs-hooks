...
class GitLfsCheckTest(unittest.TestCase):
    ...
    def run_git_remote(self, *args, **kwargs):
        """Helper to run git commands in the remote repository."""
        kwargs.setdefault("env", self.env)
        kwargs.setdefault("stdout", subprocess.PIPE)
        kwargs.setdefault("stderr", subprocess.PIPE)
        kwargs.setdefault("universal_newlines", True)
        check = kwargs.pop("check", True)
        return subprocess.run(["git", "-C", self.remote_dir] + list(args), check=check, **kwargs)
    ...
    def test_server_config_allowlisted_commits_file(self):
        """Allow pushing unamendable historical commits when listed in server-side allowlist file."""
        self.write_local_file(".gitattributes", "*.bin filter=lfs\n")
        self.write_local_file("legacy.bin", "raw binary content")
        self.run_git_local("add", ".")
        self.run_git_local("commit", "-m", "unamendable legacy commit")
        legacy_sha = self.run_git_local("rev-parse", "HEAD", stdout=subprocess.PIPE, text=True).stdout.strip()

        # Push without server allowlisting -> fails
        res_fail = self.push()
        self.assertNotEqual(res_fail.returncode, 0)
        self.assertIn("legacy.bin: matches LFS attribute but is NOT an LFS pointer", res_fail.stdout + res_fail.stderr)

        # Configure server-side allowlist file
        allowlist_file = os.path.join(self.tmp_dir, "allowlisted_commits.txt")
        with open(allowlist_file, "w", encoding="utf-8") as f:
            f.write(f"# Historical unamendable commit\n{legacy_sha}\n")
        self.run_git_remote("config", "git-obs-hooks.lfsAllowlistedRevisionFile", allowlist_file)

        # Push with server allowlisting -> succeeds
        self.env["GIT_OBS_HOOKS_VERBOSE"] = "1"
        res_pass = self.push()
        self.assertEqual(res_pass.returncode, 0, f"Push failed with allowlisting: {res_pass.stderr}")
        self.assertIn("allowlisted", res_pass.stdout + res_pass.stderr)

        # Develop on top of allowlisted revision: child commit touching only text file also succeeds
        self.write_local_file("readme.txt", "doc update")
        self.run_git_local("add", ".")
        self.run_git_local("commit", "-m", "child commit touching text file")
        res_child = self.push()
        self.assertEqual(res_child.returncode, 0, f"Push failed for child commit: {res_child.stderr}")

        # Introducing a new non-compliant file in child commit is still blocked
        self.write_local_file("new_bad.bin", "new raw binary")
        self.run_git_local("add", ".")
        self.run_git_local("commit", "-m", "new non-compliant file")
        res_bad = self.push()
        self.assertNotEqual(res_bad.returncode, 0)
        self.assertIn("new_bad.bin: matches LFS attribute but is NOT an LFS pointer", res_bad.stdout + res_bad.stderr)
